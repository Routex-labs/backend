...
낡는 것을 여기서 걸러 낸다.
  ...
  싣는다  summary  식당·시설 소개글. `※` 뒤 운영 고지는 잘라 낸다 — "LAST ORDER
                  ...
                  줄은 낡는다.
          hero     식당·시설 사진. `fetch_assets.py`가 번들에 받아 둔 파일을 가리킨다.
          menu     메뉴 이름·사진·가격·설명.
  ...
  버린다  lastOrder  `hours` 구조체에 자리가 없다. 자유 문자열로 넣으면
          ...
          사진 없는 메뉴  `menu.image_asset`이 필수다. 이름만 실으면 사진 자리가 빈
                   카드가 되는데, 그건 "사진이 없다"가 아니라 "못 불러왔다"로 읽힌다.
...
def asset(url, prefix):
    """번들에 받아 둔 사진 경로. 이름 규칙은 `fetch_assets.asset_name`과 같아야 한다 —
    갈라지면 등록은 됐는데 카드만 빈 채로 뜨고, 빌드는 성공한다."""
    if not url:
        return None
    return "assets/place_details/%s_%s" % (prefix, os.path.basename(url.split("?")[0]))


def hero(urls, prefix):
    items = [{"local_asset": asset(u, prefix)} for u in (urls or []) if u]
    return items or None


def menu_items(items):
    """메뉴 줄. `image_asset`이 필수라 사진 없는 항목은 버린다.

    이름만 남겨 실으면 화면에 사진 자리가 빈 카드가 생기는데, 그건 "사진이 없다"가
    아니라 "못 불러왔다"로 읽힌다."""
    rows, dropped = [], []
    for item in items or []:
        path = asset(item.get("image"), "menu")
        if not path:
            dropped.append(item.get("name"))
            continue
        row = {"name": item["name"], "image_asset": path}
        for key in ("price", "description"):
            if item.get(key):
                row[key] = item[key]
        rows.append(row)
    return rows, dropped
...
def main(today):
    ...
    out_rest, no_summary, no_hours, no_menu_photo = {}, [], [], []
    for store, r in r_pairs:
        ...
        if tel:
            overlay["contact"] = tel
        photos = hero(r.get("images"), "dining")
        if photos:
            overlay["hero"] = photos
        rows, dropped = menu_items(menus.get(str(r["seq"]), menus.get(r["seq"])))
        if rows:
            overlay["menu"] = rows
        no_menu_photo.extend(dropped)
        if len(overlay) > 2:
    ...
    for hits, c in f_pairs:
        ...
        for e in hits:
            ...
            photo = hero([c.get("image")], "facility")
            if photo:
                overlay["hero"] = photo
    ...
    if no_menu_photo:
        print("사진이 없어 뺀 메뉴 %d줄" % len(no_menu_photo))
    if no_tel:
